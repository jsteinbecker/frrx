...
class EmployeeForm(forms.ModelForm):
    ...
    phase_pref.help_text = (
        "Select the time phase(s) that this employee prefers to work."
    )
    phase_pref.label = "Time Phase Preference"

    streak_pref = forms.ChoiceField(choices=STREAK_CHOICES)
    streak_pref.help_text = (
        "Select the maximum number of consecutive days this employee prefers to work."
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "enrolled_in_inequity_monitoring": forms.CheckboxInput(attrs={"class": "switch"}),
            "std_hours_override": forms.Select(
                choices=STDHRS_CHOICES, attrs={"class": "select"}
            ),
        }
...
class TrainingForm(forms.Form):
    ...
    shift = forms.ModelChoiceField(
        queryset=Shift.objects.all(), widget=forms.HiddenInput()
    )
        ...
        if training == "UT":
            ...
            if training == "AV":
                if employee.shifttraining_set.filter(shift=shift).exists():
                    ...
                    ShiftTraining.objects.create(
                        employee=employee, shift=shift, is_active=True
                    )
            elif training == "UA":
                if employee.shifttraining_set.filter(shift=shift).exists():
                    tr = employee.shifttraining_set.filter(shift=shift).first()
                    ...
                    ShiftTraining.objects.create(
                        employee=employee, shift=shift, is_active=False
                    )
...
class CustomModelChoiceIterator(ModelChoiceIterator):
    def choice(self, obj):
        return self.field.prepare_value(obj), f"{obj.name} ({obj.verbose_name})"
...
class EmployeeCreateForm(forms.ModelForm):
    department = forms.ModelChoiceField(
        queryset=Department.objects.all(), widget=forms.HiddenInput())
    fte = forms.DecimalField(max_digits=4, decimal_places=3, initial=1.0, min_value=0.0, max_value=1.0,
        widget=forms.NumberInput(attrs={"class": "form-control", "step": 0.125, "min": 0.0, "max": 1.0}),)
    start_date = forms.DateField(widget=forms.DateInput(attrs={"type": "date"}))
    phase_pref = CustomModelChoiceField(queryset=TimePhase.objects.all(),
    ...
    class Meta:
        ...
        labels = dict(
            enrolled_in_inequity_monitoring="Watch Inequity?",
            std_hours_override="Override FTE Hours?")
        help_texts = dict(
            pto_hours="Used as the per day hour value for a PTO request.",
            enrolled_in_inequity_monitoring="When selected, monitoring will occur to help distribute <br/>"
                                            "unfavorable shifts among them and their colleagues")
